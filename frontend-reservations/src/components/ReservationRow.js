...
const ReservationRow = ({reservation}) => {
    ...
    const {updateGuestStatus, roomList, fetchAvailableRoomsByDate, updateGuestRoom} = useContext(HotelContext);
    const [updatedStatus, setUpdatedStatus] = useState("");
    const [availableRooms, setAvailableRooms] = useState([]);

    useEffect(() => {
        fetchAvailableRoomsByDate(new Date(checkIn), new Date(checkOut)).then(result => setAvailableRooms(result))
    }, []);
